...
/** Herkunft (ADR-021): Der Tutor hängt `von=tutor` an jeden Link; die Seite zeigt dann "Zurück zu Claude". */
const HERKUNFT = { name: "von", werte: ["tutor"] };
...
export function pruefeLink(datei, link, seiten) {
  ...
  for (const [name, wert] of link.parameter) {
    ...
    } else if (seite.werte?.[name]) {
      // Feste Werte (modus, von) sind nie Platzhalter.
      if (!seite.werte[name].includes(wert)) fehler.push(`${datei}: ${link.roh} – ${name}=${wert} (erlaubt: ${seite.werte[name].join(", ")})`);
...
/** Kopie von `seiten`, in der jede HTML-Seite zusätzlich `von=tutor` annimmt (und nur diesen Wert). */
export function erlaubeHerkunft(seiten) {
  return new Map([...seiten].map(([seite, info]) => [seite, seite.endsWith(".html")
    ? { ...info, parameter: [...info.parameter, HERKUNFT.name], werte: { ...info.werte, [HERKUNFT.name]: HERKUNFT.werte } }
    : info]));
}

/** `von` gehört der Herkunft; ein Generator, der ihn als Aufgabenparameter liest, bräche den Vertrag. */
export function herkunftKollision(seite, parameter) {
  return parameter.includes(HERKUNFT.name) ? [`${seite}: der Generator belegt den Parameter ${HERKUNFT.name}, der für von=tutor reserviert ist`] : [];
}
...
export function pruefeVorgaben(datei, link, modul, { leseVorgaben, erzeugeZufall }, alternativen = new Map()) {
  const eigene = link.parameter.filter(([name, wert]) => !AUFGABENNUMMER.includes(name) && name !== HERKUNFT.name && !PLATZHALTER.test(wert));
