 ...
 *   node lib/pruefe-ausgabe.js build/microsite/output --ausser lunrjsindex.js
 * --ausser <dateiname> überspringt reine Daten, etwa den Suchindex, der Doku-Text mit "@import url(//…)" enthält.
 */
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  const [ordner, ...optionen] = process.argv.slice(2);
  const ausser = new Set(optionen.filter((_, i) => optionen[i - 1] === "--ausser"));
  if (!ordner || !fs.existsSync(ordner)) {
    console.error(`Aufruf: node lib/pruefe-ausgabe.js <ordner> (${ordner ?? "kein Ordner"} fehlt)`);
    process.exit(2);
  }
  const dateien = alleDateien(ordner).filter((p) => !ausser.has(path.basename(p)));
