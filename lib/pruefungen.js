 ...
 * Erlaubt sind die eigene Site, Dateien und Ordner im eigenen Repository (keine Issues: die kann jeder nach dem Review
 * ändern), serlo-Artikel und YouTube-Videos (die llms.txt nennt die Videos der Kompetenzseiten); relative Links ohnehin. Jedes andere Ziel bricht den Build.
 */
export function erlaubteTutorZiele(site) {
  return [site.basis, `${site.repo}/blob/`, `${site.repo}/tree/`, "https://de.serlo.org/", "https://www.youtube.com/watch?v="];
}

// Absolute URLs (jedes Schema, auch mit nur einem / oder \\), protokoll-relative //host, www.host ohne Schema,
// javascript:/data:/vbscript:/mailto: und nackte Hosts mit Pfad wie evil.com/x (ein bloßer Name wie serlo.org ist kein Link).
