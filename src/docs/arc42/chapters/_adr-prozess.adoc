// ADR-026: KI-Review als Pflichtschritt vor dem Merge (#24, EPIC #18).
...
[[adr-026]]
=== ADR-026: KI-Review in frischem Kontext als Pflichtschritt vor dem Merge
...
Commit kam, den Kopf-Commit nennt und „freigegeben“ sagt. Jeder neue Commit macht ihn wieder rot. Der
Check ruft kein Sprachmodell auf, braucht kein Secret und liest nur, mit dem `GITHUB_TOKEN` und
Leserechten. Er startet bei `pull_request` und `pull_request_review`: Ein `issue_comment`-Lauf hinge
am letzten Commit von `main` und erschiene nicht im PR. Pflicht im Branch-Schutz macht ihn der Product
...
Skript kommen bei `pull_request` aus dem PR-Zweig; ein PR könnte den Check selbst aufweichen, deshalb
