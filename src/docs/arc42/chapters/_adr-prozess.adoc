...
werden ignoriert) und #60 (`:imagesdir:`; der Wrapper filtert IMG001 weiter).
...
sie nicht. Dependency Review braucht den Abhängigkeitsgraphen von GitHub und ist kein Pflicht-Check; ein Ausfall
von GitHubs Dienst hält keinen Merge auf. Pflicht würde er mit einem weiteren Eintrag `abhaengigkeiten` in
`required_status_checks` (wie bei M-23). Die Action ist ein Werkzeug von außen im PR-Lauf mit Leserechten
