...
*Entscheidung.* Das Kind schreibt zu jeder Aufgabe die Aufgabe und den ganzen Rechenweg auf Papier, wie
in der Klassenarbeit. Es fotografiert nur das Blatt, ohne Namen, und schickt das Foto in den Chat. Der
Tutor prüft jeden Schritt, nennt den ersten falschen und stellt eine Leitfrage, statt die Korrektur zu
geben. Das Ergebnis tippt das Kind wie bisher in die App; die App prüft es. Die Regel steht in
`tutor.md` (Abschnitt 3, „Rechenweg auf Papier“) und in `llms.txt` jeder App („Regeln für den Tutor“)
sowie in der Übersicht `/llms.txt`. Die App ändert sich nicht.
...
Ein Rechenweg-Feld bliebe im Browser, und kein Foto verließe das Gerät (QZ-1 +1). Das Kind müsste aber
