...
/** true, wenn der Tutor die Seite geöffnet hat (`von=tutor`, genau so geschrieben). */
export function vomTutor(query) {
  return new URLSearchParams(query || "").get("von") === "tutor";
}

/** Stand der aktuellen Aufgabe als Text. stand: { versuche, richtigImVersuch?, loesungGezeigt? }. */
...
export function zurueckKnopf({ zeile, feld }) {
  ...
  const textFeld = feld ?? eigenesFeld;
