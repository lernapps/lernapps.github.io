...
function parser(tokens) {
  ...
  function basis() {
    ...
    if (ist("zahl")) {
      const d = dezimalZuBruch(tokens[i].text);
      i++;
      if (!d) return null;
      let wert = d.wert;
      if (ist("prozent")) {
  ...
  function faktor() {
    ...
    if (ist("op", "^")) {
      i++;
      if (!ist("zahl") || !/^\d+$/.test(tokens[i].text)) return null;
  ...
  function term() {
    ...
    while (tokens[i] && tokens[i].typ === "op" && "*/:·×".includes(tokens[i].text)) {
...
export function leseTerm(text) {
  ...
  try {
    ...
    return { wert: exakt ? r.wert : null, zahl: alsZahl(r.wert), exakt, stellen: r.stellen };
  } catch (e) {
    return { fehler: e instanceof TermFehler ? e.message : "ungueltig" };
  }
}
