/*
 * Kompetenz 3: Baumdiagramm zeichnen – einige Zweigwahrscheinlichkeiten sind versteckt (a, b, c, d), die Lernende
...
import { pruefeWahrscheinlichkeit, wahrscheinlichkeitsFeld, versuchAusVorgaben, versuchText, zweigBruch, VERSUCH_ZAHLEN, VERSUCH_TEXTE, MELDUNG_UNLESBAR } from "./gemeinsam.js";
...
function waehleVersteckte(baum, zufall) {
  ...
  return gewaehlt.map((k, i) => {
    ...
    const beschreibung = `${k.stufe}. Zug: ${k.name}${vorher.length ? ` (nach ${vorher.join(", ")})` : ""}`;
...
export function erzeugeAufgabe(zufall, vorgaben = {}) {
  ...
  return {
    ...
    tipp: mitZuruecklegen
      ? "Jeder Zweig zeigt: Anzahl der passenden Kugeln (oder Seiten) geteilt durch alle. Mit Zurücklegen bleibt das auf jeder Stufe gleich. Die Zweige an einem Knoten ergeben zusammen 1."
