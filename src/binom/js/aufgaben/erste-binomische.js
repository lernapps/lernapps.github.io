/*
 * Kompetenz: Erste binomische Formel – (a + b)² = a² + 2ab + b².
...
import { multipliziere, alsEingabe, alsAnzeige, gliedAnzeige, binomAnzeige } from "./terme.js";
...
export const THEMA = "erste-binomische";
...
export function erzeugeAufgabe(zufall, vorgaben = {}) {
  ...
  const { u, v } = binom;
  const quadrat = `${binomAnzeige([u, v])}²`;
  const mitte = multipliziere([u], [v]).map((g) => ({ ...g, k: 2 * g.k }))[0];
  const ergebnis = multipliziere([u, v], [u, v]);
  return {
    thema: THEMA,
    ...binom,
    mitte,
    ergebnis,
    aufgabeText: quadrat,
    text: `Multipliziere mit der ersten binomischen Formel aus: ${quadrat}`,
    felder: [{ id: "antwort", typ: "variablenterm", label: `${quadrat} =`, hinweis: TERM_HINWEIS }],
    loesung: { antwort: alsEingabe(ergebnis) },
    tipp: `Erstes Glied a = ${gliedAnzeige(u)}, zweites Glied b = ${gliedAnzeige(v)}. Rechne a², dann 2 · a · b, dann b².`,
    rechenweg: [
...
function meldungen(aufgabe) {
  return {
    ...
    "binom-vergessen": `Da fehlt das Mittelglied 2ab. ${aufgabe.aufgabeText} heißt ${binomAnzeige([aufgabe.u, aufgabe.v])} · ${binomAnzeige([aufgabe.u, aufgabe.v])} – das sind vier Produkte, nicht zwei.`,
...
export function pruefeAntwort(aufgabe, antworten) {
  ...
  const fehler = ergebnis.fehler !== "falsch" ? ergebnis.fehler : diagnose(eingabe, richtig, [
    ["binom-vergessen", [[uu, vv]]],
    ["faktor-2-vergessen", [[uu, { ...mitte, k: mitte.k / 2 }, vv]]],
    ["koeffizient-nicht-quadriert", nichtQuadriert(u, v, mitte)],
  ]);
